...
from tkinter import messagebox  # Keep messagebox from tkinter as CustomTkinter doesn't have it yet
import tkinter.scrolledtext as scrolledtext  # Keep scrolledtext from tkinter
import tkinter.simpledialog  # Keep simpledialog from tkinter
import tkinter as tk  # Keep tk for compatibility with some widgets
import subprocess
import webbrowser
import os
import sys
...
class InvokeX:
    ...
    def __init__(self, root):
        ...
        self.root.geometry("1200x800")  # Slightly larger for modern UI
        ...
        # self.setup_styles()  # No longer needed with CustomTkinter
        ...
        # Create tabview for tabs (modern replacement for Notebook)
        self.notebook = ctk.CTkTabview(main_container)
        self.notebook.grid(row=0, column=0, sticky='nsew', padx=10, pady=(10, 10))
        ...
        # Check Python availability asynchronously to avoid blocking startup
        self.python_available = None
        self.root.after(500, self._check_python_async)
        
        # Show admin status asynchronously to avoid blocking
        if not self.is_admin:
            self.root.after(1000, self.show_admin_warning)
    
    def _check_python_async(self):
        """Check Python availability asynchronously to avoid blocking the UI."""
        try:
            import subprocess
            ...
            if result.returncode == 0 and result.stdout:
                ...
                self.log_to_terminal(f"Python detected: {version}", "SUCCESS")
            else:
                self.python_available = False
                self.log_to_terminal("Python not found in system PATH", "WARNING")
        except Exception as e:
            self.python_available = False
            self.log_to_terminal(f"Python check failed: {str(e)}", "WARNING")
    ...
    def setup_styles(self):
        ...
        # CustomTkinter handles all styling automatically
        # No manual style configuration needed
        pass
    ...
    def show_admin_warning(self):
        ...
        try:
            warning_frame = ctk.CTkFrame(
                self.root, 
                fg_color=("#fff3cd", "#4a4a4a"),
                corner_radius=8,
                border_width=1,
                border_color=("#ffeaa7", "#6c757d")
            )
            ...
            restart_btn = ctk.CTkButton(
                ...
                hover_color=("#c82333", "#9b2332"),
                font=ctk.CTkFont(size=11, weight="bold"),
                corner_radius=6,
                width=120,
                height=28
            )
            restart_btn.pack(side='left', padx=(0, 10), pady=8)
            ...
            self.root.after(10000, lambda: self._safe_destroy(warning_frame))
        except Exception as e:
            # If warning fails, just log it - don't crash the app
            self.log_to_terminal(f"Warning banner failed: {str(e)}", "DEBUG")
    ...
    def restart_as_admin(self):
        ...
        try:
            ...
            try:
                ...
                self.log_to_terminal(f"Executing PowerShell command: {ps_command}", "info")
                
                # Execute the PowerShell command
                result = subprocess.run(['powershell', '-Command', ps_command], 
                                      capture_output=True, text=True, shell=True, timeout=15)
                
                self.log_to_terminal(f"PowerShell result - Return code: {result.returncode}", "info")
                if result.stdout:
                    self.log_to_terminal(f"PowerShell stdout: {result.stdout.strip()}", "info")
                if result.stderr:
                    self.log_to_terminal(f"PowerShell stderr: {result.stderr.strip()}", "warning")
                
                if result.returncode == 0:
                    # Close the current application
                    self.log_to_terminal("PowerShell command executed successfully. Closing current instance...", "success")
                    self.root.after(2000, self.root.quit)  # Increased delay
                else:
                    raise Exception(f"PowerShell command failed with code {result.returncode}: {result.stderr}")
                
            except Exception as e:
                # Method 2: Fallback to creating a batch file that uses pythonw
                self.log_to_terminal(f"PowerShell method failed ({str(e)}), trying batch file method...", "warning")
                
                # Create a batch file to restart as admin with pythonw (no console)
...
'''
                ...
                try:
                    ...
                    self.log_to_terminal(f"Created batch file: {batch_path}", "info")
                    
                    # Run the batch file as admin using PowerShell
                    batch_cmd = f'Start-Process -FilePath "{batch_path}" -Verb RunAs'
                    batch_result = subprocess.run(['powershell', '-Command', batch_cmd], 
                                                capture_output=True, text=True, shell=True, timeout=10)
                    
                    self.log_to_terminal(f"Batch execution result: {batch_result.returncode}", "info")
                    if batch_result.stderr:
                        self.log_to_terminal(f"Batch stderr: {batch_result.stderr.strip()}", "warning")
                    
                    # Clean up batch file after a delay
                    self.root.after(3000, lambda: os.remove(batch_path) if os.path.exists(batch_path) else None)
                    
                    # Close the current application
                    self.log_to_terminal("Batch method executed. Closing current instance...", "success")
                    self.root.after(2500, self.root.quit)
                    
                except Exception as batch_error:
                    self.log_to_terminal(f"Batch method also failed: {str(batch_error)}", "error")
                    
                    # Method 3: Last resort - try direct elevation with UAC prompt
                    self.log_to_terminal("Trying direct UAC elevation as last resort...", "warning")
                    try:
                        import ctypes
                        ...
                        self.log_to_terminal("Direct UAC elevation attempted. Closing current instance...", "success")
                        self.root.after(2000, self.root.quit)
                    except Exception as uac_error:
                        self.log_to_terminal(f"All restart methods failed: {str(uac_error)}", "error")
                        raise uac_error
    ...
    def create_terminal_window(self, parent):
        ...
        """
        # Terminal frame with modern styling - smaller height
        terminal_frame = ctk.CTkFrame(parent)
        terminal_frame.grid(row=1, column=0, sticky='nsew', padx=10, pady=(5, 10))
        terminal_frame.grid_columnconfigure(0, weight=1)
        terminal_frame.grid_rowconfigure(1, weight=1)
        
        # Terminal header
        terminal_header = ctk.CTkLabel(terminal_frame, text="Terminal Output", 
                                      font=ctk.CTkFont(size=14, weight="bold"))
        terminal_header.grid(row=0, column=0, sticky='w', padx=15, pady=(10, 5))
        
        # Terminal text widget with smaller height and better scrolling
        ...
        self.terminal.grid(row=1, column=0, sticky='nsew', padx=15, pady=5)
        
        # Clear button frame for bottom-right positioning
        button_frame = ctk.CTkFrame(terminal_frame, fg_color="transparent")
        button_frame.grid(row=2, column=0, sticky='se', padx=15, pady=(0, 10))
        
        # Clear button - smaller, red, bottom right
    ...
    def log_to_terminal(self, message, level="INFO"):
        ...
        # Emoji/symbol map for different log levels for visual feedback
        symbol_map = {
            "INFO": "ℹ",        # Info symbol
            "ERROR": "❌",       # Red X
        ...
        symbol = symbol_map.get(level.upper(), "•")
        
        # Enhanced terminal output with symbols and better formatting
        log_entry = f"[{timestamp}] {symbol} {level.upper():7} | {message}\n"
        
        # Add to terminal (CustomTkinter textbox doesn't support color tags)
        try:
            # Get current content
            current_content = self.terminal.get("1.0", "end-1c")
            
            # Add new entry
            new_content = current_content + log_entry if current_content else log_entry
            
            # Update terminal content
            self.terminal.delete("1.0", "end")
            self.terminal.insert("1.0", new_content)
    ...
    def create_apps_tab(self):
        ...
        # Add the tab and get reference to it
        self.notebook.add("Applications")
        apps_frame = self.notebook.tab("Applications")
        ...
        # Title in header
        title_label = ctk.CTkLabel(header_frame, text="Application Installer", 
                                  font=ctk.CTkFont(size=24, weight="bold"))
        title_label.grid(row=0, column=0, pady=20, sticky='w', padx=30)
        
        # Subtitle
        subtitle_label = ctk.CTkLabel(header_frame, text="Install and manage essential Windows applications", 
                                     font=ctk.CTkFont(size=14),
                                     wraplength=900)
        subtitle_label.grid(row=1, column=0, pady=(0, 20), sticky='w', padx=30)
        
        # Create modern scrollable frame for apps
        apps_container = ctk.CTkScrollableFrame(apps_frame)
        apps_container.grid(row=2, column=0, sticky='nsew', padx=20, pady=(10, 20))
    ...
    def create_tweaks_tab(self):
        ...
        # Add the tab and get reference to it
        self.notebook.add("System Tweaks")
        tweaks_frame = self.notebook.tab("System Tweaks")
        ...
        header_frame = ctk.CTkFrame(tweaks_frame)
        header_frame.grid(row=0, column=0, sticky='ew', padx=0, pady=0)
        header_frame.grid_columnconfigure(0, weight=1)
        
        # Title in header
        title_label = ctk.CTkLabel(header_frame, text="System Tweaks", 
                                  font=ctk.CTkFont(size=24, weight="bold"))
        title_label.grid(row=0, column=0, pady=20, sticky='w', padx=30)
        
        # Subtitle with Windows version
        windows_version = self.get_windows_version()
        subtitle_label = ctk.CTkLabel(header_frame, text=f"Customize Windows settings and behavior • {windows_version}", 
                                     font=ctk.CTkFont(size=14),
                                     wraplength=900)
        subtitle_label.grid(row=1, column=0, pady=(0, 20), sticky='w', padx=30)
        
        # Create modern scrollable frame for tweaks
        tweaks_container = ctk.CTkScrollableFrame(tweaks_frame)
        tweaks_container.grid(row=2, column=0, sticky='nsew', padx=20, pady=(10, 20))
    ...
    def create_single_tweak_with_3_buttons(self, parent, title, description, btn1_text, btn2_text, btn3_text, btn1_func, btn2_func, btn3_func):
        ...
        tweak_frame = ctk.CTkFrame(parent, fg_color=("gray90", "gray13"), corner_radius=12)
        tweak_frame.pack(fill='x', pady=6, padx=8)
        
        # Tweak info with consistent padding - more condensed
        ...
        # Buttons frame with modern alignment
        ...
        btn1 = ctk.CTkButton(buttons_frame, text=btn1_text, 
                            command=btn1_func,
                            fg_color=("#1f538d", "#14375e"),
                            hover_color=("#1c4c84", "#0d2a42"),
                            corner_radius=8,
                            height=36,
                            width=140,
        ...
        btn2 = ctk.CTkButton(buttons_frame, text=btn2_text, 
                            command=btn2_func,
                            fg_color=("#157347", "#0d4b2b"),
                            hover_color=("#146c41", "#0a3f24"),
                            corner_radius=8,
                            height=36,
                            width=140,
                            font=ctk.CTkFont(weight="bold"))
        ...
        if btn3_text and btn3_func:
            btn3 = ctk.CTkButton(buttons_frame, text=btn3_text, 
                                command=btn3_func,
                                fg_color=("#17a2b8", "#0c5460"),
                                hover_color=("#138496", "#087990"),
                                corner_radius=8,
                                height=36,
                                width=140,
                                font=ctk.CTkFont(weight="bold"))
    ...
    def check_app_async(self, app_name, status_label):
        ...
        try:
            ...
            status_label.configure(text="○ Not Installed", text_color="#95a5a6")
    ...
    def check_mass_activation_async(self, status_label):
        ...
        try:
            ...
            status_label.configure(text="✗ Windows Not Activated", text_color="#e74c3c")
    ...
    def check_ninite_async(self, status_label):
        ...
        try:
            ...
            status_label.configure(text="✗ Apps Installed (0/4)", text_color="#e74c3c")
    ...
    def create_app_section(self, parent, title, description, button_text, github_url, install_func):
        ...
        """
        # Container for each app with modern card styling
        app_frame = ctk.CTkFrame(parent, fg_color=("gray90", "gray13"), corner_radius=12)
        app_frame.pack(fill='x', pady=6, padx=8)
        
        # App info with consistent padding - more condensed
        ...
        install_btn = ctk.CTkButton(buttons_frame, text=button_text, 
        ...
        website_btn = ctk.CTkButton(buttons_frame, text=button_label, 
    ...
    def create_app_section_with_3_buttons(self, parent, title, description, btn1_text, btn2_text, btn3_text, btn1_url, install_func, btn2_func, btn3_func):
        ...
        install_btn = ctk.CTkButton(buttons_frame, text=btn1_text, 
                                   command=install_and_refresh,
                                   fg_color=("#1f538d", "#14375e"),
                                   hover_color=("#1c4c84", "#0d2a42"),
                                   corner_radius=8,
        ...
        # Button 2 (GitHub/Link button - Secondary action)
        btn2 = ctk.CTkButton(buttons_frame, text=btn2_text, 
                            command=btn2_func,
                            fg_color=("#6c757d", "#495057"),
                            hover_color=("#5a6268", "#343a40"),
        ...
        # Button 3 (Logs button - Success color)
        btn3 = ctk.CTkButton(buttons_frame, text=btn3_text, 
                            command=btn3_func,
                            fg_color=("#157347", "#0d4b2b"),
                            hover_color=("#146c41", "#0a3f24"),
    ...
    def view_power_logs(self):
        ...
        try:
            ...
            # Pack text widget (scrolling handled by scrolledtext automatically)
    ...
    def restart_system_10s(self):
        ...
        countdown_window = ctk.CTkToplevel(self.root)
        ...
        # Countdown label
        countdown_label = ctk.CTkLabel(countdown_window, text="System will restart in:", 
                                      font=ctk.CTkFont(size=14, weight="bold"))
        countdown_label.pack(pady=(20, 10))
        
        # Time label
        self.time_label = ctk.CTkLabel(countdown_window, text="15", 
                                      font=ctk.CTkFont(size=28, weight="bold"), 
                                      text_color=("#e74c3c", "#ff6b6b"))
        self.time_label.pack(pady=(0, 20))
        
        # Cancel button
        cancel_btn = ctk.CTkButton(countdown_window, text="Cancel Restart", 
    ...
    def get_admin_password(self):
        ...
        # CustomTkinter handles dialog styling automatically
        
        # Title label
        title_label = ctk.CTkLabel(password_dialog, 
                                  text="Set Password for Admin Account", 
                                  font=ctk.CTkFont(size=18, weight="bold"))
        title_label.pack(pady=(20, 10))
        
        # Info label
        info_label = ctk.CTkLabel(password_dialog, 
        ...
        # Password frame
        password_frame = ctk.CTkFrame(password_dialog, fg_color="transparent")
        ...
        confirm_frame = ctk.CTkFrame(password_dialog, fg_color="transparent")
        ...
        confirm_entry = ctk.CTkEntry(confirm_frame, 
        ...
        # Buttons frame
        buttons_frame = ctk.CTkFrame(password_dialog, fg_color="transparent")
        buttons_frame.pack(pady=20)
        
        # OK button
        ok_button = ctk.CTkButton(buttons_frame, 
                                 text="Create Account", 
